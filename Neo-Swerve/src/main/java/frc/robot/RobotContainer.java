...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    swerve.setDefaultCommand(swerve.drive(
      () -> -Constants.kControls.Y_DRIVE_LIMITER.calculate((Math.pow(xbox.getLeftY(), 3) / Constants.kSwerve.SENSITIVITY - 0.05)), 
      () -> -Constants.kControls.X_DRIVE_LIMITER.calculate((Math.pow(xbox.getLeftX(), 3) / Constants.kSwerve.SENSITIVITY - 0.075)),  
      () -> Constants.kControls.THETA_DRIVE_LIMITER.calculate(xbox.getRightX() - 0.025),
      true,
      false
      ));
